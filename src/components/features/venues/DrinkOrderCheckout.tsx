...
interface DrinkOrderCheckoutProps {
  items: any[];
  venue: any;
  onComplete: () => void;
  onBack: () => void;
...
export const DrinkOrderCheckout: React.FC<DrinkOrderCheckoutProps> = ({ items: initialItems, venue, onComplete, onBack }) => {
  const [items, setItems] = useState(initialItems.map(item => ({ ...item, quantity: 1, points: 10 })));
